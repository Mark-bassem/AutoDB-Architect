...
const examples = [
  "عايز سيستم لمستشفى فيه Doctors و Patients و Appointments",
  "Uber-like app with Riders, Drivers, Trips, and Payments.",
  "Online School with Students, Courses, Instructors, and Enrollments."
];

const tabs = [
  { id: 'erd', label: 'مخطط ERD', icon: 'layout', content: renderERD },
  { id: 'sql', label: 'سكربت SQL', icon: 'terminal', content: renderSQL },
  { id: 'code', label: 'نماذج Python', icon: 'code', content: renderPython },
  { id: 'json', label: 'JSON الخام', icon: 'file-json', content: renderJSON },
];
  ...
  const fieldsHtml = (entity.fields || []).map(f => `
    <div class="flex justify-between items-center text-sm py-1.5 border-b border-slate-700/50 last:border-0 hover:bg-slate-700/30 px-1 rounded">
      <div class="flex items-center gap-2">
        ${f.isPK ? '<span title="مفتاح أساسي" class="text-yellow-500 text-xs">PK</span>' : ''}
        ${f.isFK ? '<span title="مفتاح أجنبي" class="text-purple-500 text-xs">FK</span>' : ''}
...
function renderSQL(schema) {
  ...
  return `<div><div class="flex items-center justify-between mb-2"><h3 class="text-lg font-semibold text-slate-200">مخطط قاعدة البيانات (DDL)</h3><span class="text-xs text-slate-500 bg-slate-800 px-2 py-1 rounded">متوافق مع PostgreSQL / MySQL</span></div>${createCodeBlock(sqlCode, 'sql')}</div>`;
}

function renderPython(schema) {
  if (!schema) return '';
  const pythonCode = generatePythonClasses(schema);
  return `<div><h3 class="text-lg font-semibold text-slate-200 mb-2">نماذج كلاسات Python</h3>${createCodeBlock(pythonCode, 'python')}</div>`;
}

function renderJSON(schema) {
  if (!schema) return '';
  const jsonCode = JSON.stringify(schema, null, 2);
  return `<div><h3 class="text-lg font-semibold text-slate-200 mb-2">JSON الخام</h3>${createCodeBlock(jsonCode, 'json')}</div>`;
}
...
document.addEventListener('click', (e) => {
  ...
  }).catch(() => showError('فشل النسخ — تأكد من أذونات المتصفح.'));
});
...
async function handleGenerate() {
  ...
  if (!prompt) {
    showError('من فضلك اكتب وصف النظام أولاً.');
    return;
  }
  hideError();
  setLoading(true);
  ...
  try {
    ...
    showError('فشل في توليد المخطط. تأكد من إعداد الخادم (backend) والمفتاح.');
...
function init() {
  ...
  if (exampleHospitalBtn) {
    exampleHospitalBtn.addEventListener('click', () => {
      promptInput.value = "مستشفى يحتوي على Doctors, Patients, Appointments, Departments, Nurses, Prescriptions. كل مريض ممكن يكون له عدة Appointments. الأطباء يعملون في Departments.";
    });
  ...
  downloadSQLBtn.addEventListener('click', () => {
    if (!currentSchema) { showError('لا يوجد مخطط للتحميل'); setTimeout(hideError, 2000); return; }
